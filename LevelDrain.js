...
Game_Battler.prototype.levelDrain = function(amount, target) {
	//var actor = $gameActors._data[actorId];	
	if(target.level <= amount) {
		amount = target.level - 1;
	}
	if (amount > 0){	
...
Game_Battler.prototype.displayStatsUp = function(){
	var stats = this.name() +' ' + TextManager.level + ' ' + (this.level - this.lvlGain) 
	if (lvlGain > 0) {
		stats += ' + \\c[24]' + this.lvlGain + '\\c[0]\n';
	}
	stats += TextManager.param(0) + ':  ' + this.oldParams[0];
	var paramGain = this.param(0) - this.oldParams[0];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]  ';
	}
	...
	stats += TextManager.param(1) + ':  ' + this.oldParams[1];
	paramGain = this.param(1) - this.oldParams[1];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]\n';
	}
	...
	stats += TextManager.param(2) + ':  ' + this.oldParams[2];
	paramGain = this.param(2) - this.oldParams[2];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]  ';
	}
	...
	stats += TextManager.param(4) + ':  ' + this.oldParams[4];
	paramGain = this.param(4) - this.oldParams[4];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]  ';
	}
	...
	stats += TextManager.param(5) + ':  ' + this.oldParams[5];
	paramGain = this.param(5) - this.oldParams[5];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]\n';
	}
	...
	stats += TextManager.param(3) + ':  ' + this.oldParams[3];
	paramGain = this.param(3) - this.oldParams[3];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]  ';
	}
	...
	stats += TextManager.param(6) + ':  ' + this.oldParams[6];
	paramGain = this.param(6) - this.oldParams[6];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]  ';
	}
	...
	stats += TextManager.param(7) + ':  ' + this.oldParams[7];
	paramGain = this.param(7) - this.oldParams[7];
	if (paramGain > 0) {
		stats += ' +\\c[24] ' + paramGain + '\\c[0]';
	}
